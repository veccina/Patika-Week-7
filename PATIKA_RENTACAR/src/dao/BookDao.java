...
public class BookDao {
    ...
    public ArrayList<Book> findAll() { return this.selectByQuery("SELECT * FROM public.book ORDER BY book_id ASC"); }
    ...
    public Book getById(int id) {
        String query = "SELECT * FROM public.book WHERE book_id = " + id;
        try {
            ResultSet rs = this.con.createStatement().executeQuery(query);
            if (rs.next()) {
    ...
    public boolean delete(int id) {
        String query = "DELETE FROM public.book WHERE book_id = " + id;
        try {
            int result = this.con.createStatement().executeUpdate(query);
            return result > 0;
        } catch (SQLException throwable) {
            throwable.printStackTrace();
        }
